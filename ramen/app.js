(() => {
  ...
  const IMAGE_CACHE = 'michael-ramen-image-cache-v4';
  ...
  const text = {
    en: {
      ...
      source:'Evidence source', buy:'Buy in Germany', noBuy:'Germany buy link pending', details:'Show all data', hide:'Hide data', market:'Market', gtin:'GTIN', style:'Style', spice:'Spice', checked:'Checked', verification:'Verification', imageSource:'Image source',
      unranked:'unranked', results:'visible', fallback:'illustration', packshot:'packshot', database:'database image', searching:'finding image', noResults:'No products match these filters.',
      copied:'Ranking copied.', reset:'Ranking reset.', moved:'ranked', tastedToast:'Marked as tasted.', untastedToast:'Removed from tasted list.',
    ...
    de: {
      ...
      source:'Nachweis', buy:'In Deutschland kaufen', noBuy:'Deutscher Kauflink noch offen', details:'Alle Daten zeigen', hide:'Daten schließen', market:'Markt', gtin:'GTIN', style:'Stil', spice:'Schärfe', checked:'Geprüft', verification:'Prüfstufe', imageSource:'Bildquelle',
      unranked:'noch offen', results:'sichtbar', fallback:'Illustration', packshot:'Packshot', database:'Datenbankbild', searching:'Bildsuche', noResults:'Keine Produkte passen zu diesen Filtern.',
      copied:'Ranking kopiert.', reset:'Ranking zurückgesetzt.', moved:'eingeordnet', tastedToast:'Als probiert markiert.', untastedToast:'Aus „probiert“ entfernt.',
  ...
  function card(item) {
    ...
    const buyLink = item.buy ? `<a class="buy" href="${escapeHTML(item.buy)}" target="_blank" rel="noopener">${tr('buy')} ↗</a>` : `<span class="pending">${tr('noBuy')}</span>`;
    return `<article class="product" draggable="true" data-id="${escapeHTML(item.id)}">
      ...
      <div><span class="brandline">${escapeHTML(item.brand)} · ${escapeHTML(item.country || '')}</span><h4>${escapeHTML(item.name)}</h4>
        ...
        <details class="details"><summary data-open="${escapeHTML(tr('details'))}" data-close="${escapeHTML(tr('hide'))}">${escapeHTML(tr('details'))}</summary><dl>
          <dt>${tr('market')}</dt><dd>${escapeHTML(item.market || '—')}</dd><dt>${tr('gtin')}</dt><dd>${escapeHTML(item.gtin || '—')}</dd><dt>${tr('style')}</dt><dd>${escapeHTML(item.style || '—')}</dd><dt>${tr('spice')}</dt><dd>${escapeHTML(item.spice ?? 0)}/5</dd><dt>${tr('verification')}</dt><dd>${escapeHTML(verification || '—')}</dd><dt>${tr('checked')}</dt><dd>${escapeHTML(item.verifiedAt || '—')}</dd><dt>${tr('imageSource')}</dt><dd>${img.source?`<a href="${escapeHTML(img.source)}" target="_blank" rel="noopener">${escapeHTML(img.source)}</a>`:'—'}</dd>
        </dl></details>
  ...
  async function hydrateImages() {
    const queue = products.filter(item => !hasRealImage(item) && item.imageQuery && !state.imageCache[item.id]?.attempted);
    let cursor = 0;
  ...
  function randomPick() {
    ...
    $('#pickLinks').innerHTML = `${item.source?`<a href="${escapeHTML(item.source)}" target="_blank" rel="noopener">${tr('source')} ↗</a>`:''}${item.buy?`<a href="${escapeHTML(item.buy)}" target="_blank" rel="noopener">${tr('buy')} ↗</a>`:''}`;
  ...
  async function copyRanking() {
    ...
    try { await navigator.clipboard.writeText(`My Germany-first Ramen 50\n\n${output}\n\nhttps://mikelninh.github.io/ramen/`); showToast(tr('copied')); } catch { showToast('Copy failed.'); }
  }
