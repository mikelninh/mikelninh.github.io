...
function closeAllModals(){qa('.modal').forEach(m=>m.hidden=true)}
function share(){const url=location.origin+location.pathname+`?case=${cur().id}&mode=interview&variant=${state.variant}`;if(navigator.clipboard&&window.isSecureContext){navigator.clipboard.writeText(url).then(()=>{q('#shareBtn').textContent='Copied ✓';setTimeout(()=>q('#shareBtn').textContent='Copy challenge',1500)})}else{prompt('Copy this challenge link:',url)}}
function nextShift(){q('#debriefModal').hidden=true;if(cur().id==='broken-calls')state.variant=(state.variant+1)%BROKEN_VARIANTS.length;else state.idx=(state.idx+1)%CASES.length;applyVariant();populateCases();reset()}
